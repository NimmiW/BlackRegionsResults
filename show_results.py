...
def plot_results(year, from_month, to_month, currency):
    ...
    if (to_month == 12):
        end_date = str(to_month + 1) + "-" + str(1) + "-01"
    else:
        end_date = str(year) + "-" + str(to_month + 1) + "-01"

    quotes = pd.read_csv("C:/Users/HP/PycharmProjects/plot_results/data/"+currency+"/DAT_MT_"+currency+"_M1_" + str(year) + ".csv")
    ...
    anormalies = pd.read_csv("results/EURUSD/all_anomalies.csv")
